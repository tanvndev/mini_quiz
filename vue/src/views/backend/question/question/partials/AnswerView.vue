<template>
  <a-card class="mt-3">
    ...
    <a-row :gutter="16" v-if="props.answers.length == 0">
    ...
    <a-row :gutter="16" v-if="props.answers.length > 0">
      <a-col :span="24" v-for="(answer, index) in props.answers" :key="answer.id" class="mb-4">
        <a-row :gutter="[16, 16]" class="items-center">
          <a-col :span="1">
            <span class="font-bold">#{{ index + 1 }}</span>
          </a-col>
          ...
          <a-col :span="2" class="text-right">
            <SwitchComponent
              ...
              uncheck-text="Sai"
              :name="`answers[${index}]is_correct`"
              :old-value="answer.is_correct != 0 ? true : false"
            />
          </a-col>
